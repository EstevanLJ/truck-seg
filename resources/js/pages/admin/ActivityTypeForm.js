...
import { defaultActivityType, yesOrNoOptions, LOCALE } from "../../constants";
...
export default function ActivityTypeForm(props) {
    const [object, setObject] = useState({ ...defaultActivityType });
    ...
    useEffect(() => {
        ...
        if (props.id) {
            axios
                .get(`/api/activity-types/${props.id}`)
                .then((response) => {
                    let formData = {
                        ...defaultActivityType,
                        ...response.data.data,
    ...
    const handleSave = () => {
        ...
        axios({
            url: `/api/activity-types${props.id ? `/${props.id}` : ""}`,
        ...
        <div className="p-4 w-screen sm:w-3/4-screen lg:w-1/2-screen">
            <div className="flex justify-between items-center">
                <p className="font-bold">
                    {props.id ? LOCALE.edit : LOCALE.add} Tipo de Atividade
                </p>
