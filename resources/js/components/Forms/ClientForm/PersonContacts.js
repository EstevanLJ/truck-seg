...
export default function PersonContacts({ contacts, reload, person }) {
    ...
    const handleDeleteDialogConfirm = () => {
        axios
            .delete(`/api/person-contacts/${deletingObject.id}`)
            .then((response) => {
    ...
    const columns = [
        { id: "type_description", label: "Tipo" },
    ...
    const renderTable = () => {
        return (
            <div className="mt-3">
                <DefaultTable rows={contacts} columns={columns} />
            </div>
        );
    };
